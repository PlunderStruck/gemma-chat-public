...
export interface ModelInfo {
  ...
  draftModel?: string
  /**
   * Which local runtime serves this model. Defaults to `mlx-vlm`, which every
   * Gemma 4 model needs: they are all multimodal (text + image + audio), and
   * mlx-lm's text-only loader cannot load their vision/audio weights (it dies
   * with "Received N parameters not in model"). mlx-vlm also does the
   * assistant-drafter MTP speculative decoding. Set `mlx-lm` only for a
   * hypothetical future text-only model.
   */
...
export const AVAILABLE_MODELS: ModelInfo[] = [
  ...
  },
  {
    ...
    description:
      'Unified multimodal (text + image + audio) with MTP speculative decoding via a paired assistant drafter for faster generation. 16GB+ RAM recommended.',
